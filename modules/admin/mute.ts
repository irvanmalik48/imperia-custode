...
async function do_mute(ctx: any, user: any, seconds: number = 0) {
  ...
  if (ctx?.time_arg) r += `is temporarily muted for ${ctx?.time_arg}!`;
  else r += `has been muted!`;
...
async function unmute_cmd(ctx: any) {
  ...
  if (!(await validateRequest(ctx, rules))) return;

  if (ctx.message?.text) {
    if (await unmute_with_query(ctx)) return;
  }
...
async function mute_cmd(ctx: any) {
  if (!(await validateRequest(ctx, MUTE_MODULE_RULES))) return;
