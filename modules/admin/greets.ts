...
const rules = ["user_is_admin", "query"] satisfies ValidateOptions[];
...
export const setWelcomeCommand: Command = {
  ...
  function: async (ctx) => {
    ...
    if ((await validateRequest(ctx, rules)) === false) return;
    ...
    if (ctx.message.text) {
      ...
      const greets: GreetItem = JSON.parse(
        await fs.readFile(storage_dir + `groups/${ctx.chat.id}.json`, "utf-8")
      );
...
export const dropWelcomeAndFarewell = (ctx: Context<any>) => {
  ...
  if (fsSync.existsSync(storage_dir + `ban_silent/${ctx.chat.id}.temp`))
    return false;
  ...
  if (ctx.message?.new_chat_members) {
    ...
    if (user.is_bot) return false;
    const greets: GreetItem = JSON.parse(
      fsSync.readFileSync(storage_dir + `groups/${ctx.chat.id}.json`, "utf-8")
    );
    ctx.reply(
      greets.farewell
        .replace("{{user_name}}", user.first_name)
