...
async function do_ban(
  ...
  if (silent) {
    if (!existsSync(silent_f)) await mkdir(silent_f);
  ...
  if (ctx?.time_arg) r += `is temporarily banned for ${ctx.time_arg}!`;
  else r += `has been banned!`;
...
async function __parse_ban_cmd(ctx: any, args: string[]) {
  ...
  if (!args[1].match(/^\d+$/)) {
    ...
    if (!user) err = `You must reply to a message!`;
...
async function __parse_tban_cmd(ctx: any, args: string[]) {
  ...
  if (2 in args) args2_is_tpat = is_time_pattern(args[2]);
  else args2_is_tpat = false;

  if (!args1_is_tpat && !args2_is_tpat) {
    err =
      "Invalid <code>[time]</code> pattern, accepted time pattern is:\n" +
...
export async function parse_tban_cmd(ctx: any) {
  ...
  if (args.length === 1) {
    ...
    await ctx.reply("Please reply to a message or type the user ID!");
  }
...
async function unban_cmd(ctx: any) {
  if (!(await validateRequest(ctx, BAN_MODULE_RULES))) return;

  if (await unban_with_user_id(ctx)) return;
