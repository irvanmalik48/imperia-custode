...
async function resolve_t_user_status(ctx: any, state: any) {
  if (state.reply_uinfo !== null) return;
...
async function resolve_t_admin_list(ctx: any, state: any) {
  if (state.admin_list !== null) return;
...
async function r_user_is_admin(ctx: any, state: any) {
  ...
  if (admins.some((admin: any) => admin.user.id === user.id)) return true;
...
async function r_query(ctx: any) {
  if (ctx.message.text.split(" ")[1]) return true;
...
async function r_replied_user_in_group(ctx: any, state: any) {
  ...
  if (uinfo && uinfo.status !== "left") return true;
...
async function r_in_supergroup(ctx: any) {
  if (ctx.message?.chat?.type === "supergroup") return true;
...
export async function validateRequest(
  ...
  for (i = 0; i < rules.length; i++) {
     ...
     */
    if (!(await invoke_rule(rules[i], ctx, cache_state))) return false;
  }
